...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:param name="IN_TUNE" />
	<xsl:param name="IN_SS" />

	<xsl:template match="batfink">
		<xsl:choose>
			<xsl:when test="$IN_TUNE!=''">
				<xsl:call-template name="getcounts">
					<xsl:with-param name="count" select="count(event//set//track[contains(tune,$IN_TUNE)])" />
					<xsl:with-param name="total" select="count(event//set//track)" />
				</xsl:call-template>

				<xsl:apply-templates select="event//set//track[contains(tune,$IN_TUNE)]" mode="fullrow">
